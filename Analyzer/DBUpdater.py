...
class DBUpdater:
    ...
    def read_krx_code(self, company200_pages):
        ...
        # 종목 당 마지막 페이지 숫자 구하기
        for i in range(company200_pages):
            url = f'https://finance.naver.com/sise/sise_market_sum.nhn?&page={i + 1}'
            with urlopen(url) as doc:  # <== 이처럼 브라우저 정보를 requests 모듈을 이용해 전송해야 한다.
                html = BeautifulSoup(doc, "lxml", from_encoding='EUC-KR')
                datas = html.find_all("a", class_="tltle")
                for data in datas:
                    code = data["href"].split("=")[-1]
                    company = data.text
                    krx = krx.append(pd.DataFrame([(code, company)], columns=['code', 'company']))
        krx = krx.reset_index(drop=True)  # 인덱스 리셋
        return krx
    ...
    def update_comp_info(self):
        ...
        with self.conn.cursor() as curs:
            ...
            if rs[0] == None or rs[0].strftime('%Y-%m-%d') < today:
                krx = self.read_krx_code(4) # 시가총액 구하기 원하는 페이지 수
