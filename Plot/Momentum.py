...
class DualMomentum:
    def __init__(self):
        ...
        self.mkt = Analyzer.Marketdata.Marketdata()
    ...
    def get_rltv_momentum(self, start_date, end_date, stock_count):
        ...
        passwrd = input()
        connection = pymysql.connect(host='localhost', user=id, passwd=passwrd, db='invest', charset='utf8')
        ...
        for _, code in enumerate(self.mkt.codes):
            sql = f"select close from daily_price " f"where code='{code}' and date='{start_date}'"
            cursor.execute(sql)
            result = cursor.fetchone()
            if (result is None):
                continue
            old_price = int(result[0])
            sql = f"select close from daily_price " f"where code='{code}' and date='{end_date}'"
            cursor.execute(sql)
            result = cursor.fetchone()
            if (result is None):
                continue
            ...
            rows.append([code, self.mkt.codes[code], old_price, new_price,
                         returns])
        ...
        connection.close()
        print(df)
        print(f"\nRelative momentum ({start_date} ~ {end_date}) : " f"{df['returns'].mean():.2f}% \n")
        return df
    ...
    def get_abs_momentum(self, rltv_momentum, start_date, end_date):
        ...
        sql = f"select max(date) from daily_price " f"where date <= '{start_date}'"
        cursor.execute(sql)
        result = cursor.fetchone()
        if (result[0] is None):
        ...
        sql = f"select max(date) from daily_price " f"where date <= '{end_date}'"
        cursor.execute(sql)
        result = cursor.fetchone()
        if (result[0] is None):
            print("{} -> returned None".format(sql))
        ...
        for _, code in enumerate(stocklist):
            ...
            new_price = int(result[0])
            returns = (new_price / old_price - 1) * 100
            rows.append([code, self.mkt.codes[code], old_price, new_price, returns])
        ...
        connection.close()
        print(df)
        print(f"\nAbasolute momentum ({start_date} ~ {end_date}) : " f"{df['returns'].mean():.2f}%")
        return